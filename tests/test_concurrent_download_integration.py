...
class TestConcurrentDownloadIntegration:
    ...
    @pytest.mark.asyncio
    async def test_download_speed_monitoring(self, config, large_segment_list, temp_dir):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            # Download segments
            results = await downloader.download_segments(large_segment_list[:10], temp_dir)
            ...
            avg_speed = sum(downloader._download_speeds) / len(downloader._download_speeds)
            # Expected speed should be around 100KB/s (100,000 bytes/s)
            assert 50000 < avg_speed < 200000, f"Average speed {avg_speed} not in expected range"
    ...
    async def test_adaptive_concurrency_adjustment(self, temp_dir):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            initial_concurrency = downloader._adaptive_concurrency
    ...
    async def test_queue_management_with_cancellation(self, config, temp_dir):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            total_processed = len(downloader._completed_segments) + len(downloader._failed_segments)
    ...
    async def test_performance_metrics_accuracy(self, config, temp_dir):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            expected_keys = [
                "adaptive_concurrency", "max_concurrency", "active_downloads",
                "recent_speeds", "average_speed", "performance_window",
                "queue_size", "completed_count", "failed_count"
            ]
