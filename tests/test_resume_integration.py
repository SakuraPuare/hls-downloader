...
class TestResumeIntegration:
    ...
    def _setup_mocked_components(
        ...
        mock_detector_instance.__aenter__ = AsyncMock(
        ...
        mock_downloader_instance.__aenter__ = AsyncMock(
        ...
        mock_resume_validator.validate_segments.return_value = (
            valid_segments,
            invalid_segments,
            missing_segments,
        )
        ...
        mock_resume_validator.get_resume_summary = Mock(
            return_value={
                ...
                "completion_percentage": (
                    len(valid_segments) / len(mock_segments) * 100
                )
                if mock_segments
                else 0,
                "valid_bytes": sum(s.size for s in valid_segments),
                "can_resume": len(invalid_segments) > 0 and len(valid_segments) > 0,
                "resume_benefit": (len(valid_segments) / len(mock_segments) * 100)
                if mock_segments
                else 0,
            }
        )
        mock_resume_validator.cleanup_invalid_files = Mock(
            return_value=len(invalid_segments)
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(manager, "_detector") as mock_detector:
                ...
                with patch.object(manager, "_downloader") as mock_downloader:
                    ...
                    mock_downloader_instance.download_segments.return_value = (
                        download_results
                    )
    ...
    async def test_resume_existing_download(self, download_config, mock_segments):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (
                ...
                mock_downloader_instance,
                mock_resume_validator,
            ) = self._setup_mocked_components(
                manager, temp_dir, segments_with_status, [remaining_segment]
            )

            with (
                patch.object(
                    manager, "_initialize_components", side_effect=mock_initialize
                ),
                patch.object(manager, "_progress_display"),
                patch.object(manager, "_merger", None),
                patch.object(
    ...
    async def test_resume_with_invalid_files(self, download_config, mock_segments):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (segments_dir / "segment_000002.ts").write_bytes(
                b"x" * 2048
            )  # Correct size
            ...
            redownload_segments = [
                deepcopy(mock_segments[0]),
                deepcopy(mock_segments[2]),
            ]
    ...
    async def test_force_restart_ignores_state(self, download_config, mock_segments):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (
                ...
                download_results,
                valid_segments=[],
                invalid_segments=fresh_segments,
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            def track_save_state(self, state):
                state_updates.append(
                    (state.status, len([s for s in state.segments if s.downloaded]))
                )
            ...
            assert (
                len(state_updates) >= 3
            )  # At least: detecting, downloading, completed
    ...
    @pytest.mark.asyncio
    async def test_resume_download_convenience_method(
        self, download_config, mock_segments
    ):
