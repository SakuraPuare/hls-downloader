...
from src.hls_downloader.download_manager import DownloadManager, DownloadManagerError, ConfigurationError
from src.hls_downloader.models import DownloadConfig, SegmentInfo, DownloadStats
...
class TestDownloadManager:
    ...
    def test_validate_config_invalid_concurrent(self):
        ...
        with pytest.raises(ConfigurationError, match="max_concurrent must be greater than 0"):
    ...
    async def test_setup_output_directory_permission_error(self):
        ...
        with pytest.raises(DownloadManagerError, match="Failed to setup output directory"):
    ...
    async def test_initialize_components_ffmpeg_not_found_auto_merge_enabled(self):
        ...
        with patch('src.hls_downloader.download_manager.VideoMerger') as mock_merger:
            ...
            with pytest.raises(DownloadManagerError, match="ffmpeg required for auto-merge"):
                await manager._initialize_components()
    ...
    async def test_download_segments(self):
        ...
        segments = [
            SegmentInfo(url="http://example.com/1.ts", index=1, filename="segment_000001.ts"),
            SegmentInfo(url="http://example.com/2.ts", index=2, filename="segment_000002.ts"),
        ]
        
        download_results = [
            SegmentInfo(url="http://example.com/1.ts", index=1, filename="segment_000001.ts", downloaded=True, size=1000),
            SegmentInfo(url="http://example.com/2.ts", index=2, filename="segment_000002.ts", downloaded=True, size=1500),
        ]
    ...
    async def test_download_segments_with_failures(self):
        ...
        download_results = [
            SegmentInfo(url="http://example.com/1.ts", index=1, filename="segment_000001.ts", downloaded=True, size=1000),
            SegmentInfo(url="http://example.com/2.ts", index=2, filename="segment_000002.ts", downloaded=False),
        ]
    ...
    def test_generate_results_success(self):
        ...
        download_results = [
            SegmentInfo(url="http://example.com/1.ts", index=1, filename="1.ts", downloaded=True, size=1000),
            SegmentInfo(url="http://example.com/2.ts", index=2, filename="2.ts", downloaded=True, size=1500),
        ]
        
        with patch('time.time', return_value=1010.0):  # 10 seconds elapsed
            results = manager._generate_results(download_results, "/tmp/output/video.mp4")
    ...
    def test_generate_results_with_failures(self):
        ...
        download_results = [
            SegmentInfo(url="http://example.com/1.ts", index=1, filename="1.ts", downloaded=True, size=1000),
            SegmentInfo(url="http://example.com/2.ts", index=2, filename="2.ts", downloaded=False),
        ]
    ...
    def test_update_config(self):
        ...
        manager = DownloadManager()
        original_concurrent = manager.config.max_concurrent
    ...
    async def test_download_hls_empty_output_dir(self):
        ...
        with pytest.raises(DownloadManagerError, match="Output directory cannot be empty"):
...
async def test_download_manager_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('src.hls_downloader.download_manager.HLSDetector') as mock_detector_class, \
            ...
            mock_downloader.get_error_summary = MagicMock(return_value={"total_errors": 0})
