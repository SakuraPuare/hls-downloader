...
class TestURLPatternDetection:
    ...
    def test_simple_numeric_pattern(self, detector):
        ...
        test_cases = [
            ("http://example.com/segment1.ts", "http://example.com/", "segment{}.ts", "ts"),
            ("https://cdn.site.com/video/chunk5.m4s", "https://cdn.site.com/video/", "chunk{}.m4s", "m4s"),
            ("http://stream.tv/media/part123.ts", "http://stream.tv/media/", "part{}.ts", "ts"),
        ]
...
class TestURLGeneration:
    ...
    def test_simple_url_generation(self, detector):
        ...
        test_cases = [
            ("http://example.com/", "segment{}.ts", 1, "http://example.com/segment1.ts"),
            ("http://example.com/", "segment{}.ts", 123, "http://example.com/segment123.ts"),
            ("https://cdn.site.com/video/", "chunk{}.m4s", 5, "https://cdn.site.com/video/chunk5.m4s"),
        ]
    ...
    def test_zero_padded_url_generation(self, detector):
        ...
        test_cases = [
            ("http://example.com/", "segment{:03d}.ts", 1, "http://example.com/segment001.ts"),
            ("http://example.com/", "segment{:03d}.ts", 123, "http://example.com/segment123.ts"),
            ("http://example.com/", "chunk{:04d}.m4s", 5, "http://example.com/chunk0005.m4s"),
        ]
    ...
    def test_complex_pattern_generation(self, detector):
        ...
        expected_url = "https://cdn.example.com/live/stream/2023/12/segment42.ts?token=abc123"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(manager, '_detector') as mock_detector:
                ...
                with patch.object(manager, '_downloader') as mock_downloader:
                    ...
                    mock_downloader_instance.download_segments.side_effect = cntv_download
    ...
    async def test_generic_cdn_patterns(self, integration_config):
        ...
        for pattern in cdn_patterns:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                test_segments = [
                    SegmentInfo(
                        url=pattern.format(i) if "{}" in pattern else pattern.replace("{:04d}", str(i).zfill(4)),
                        index=i,
                ...
                with patch.object(manager, '_detector') as mock_detector:
                    ...
                    with patch.object(manager, '_downloader') as mock_downloader:
                        ...
                        mock_downloader_instance.download_segments.side_effect = cdn_download
                        mock_downloader.__aenter__.return_value = mock_downloader_instance
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with patch.object(manager, '_detector') as mock_detector:
                    ...
                    with patch.object(manager, '_downloader') as mock_downloader:
                        ...
                        mock_downloader_instance.download_segments.side_effect = auth_download
                        mock_downloader.__aenter__.return_value = mock_downloader_instance
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with patch.object(manager, '_detector') as mock_detector:
                    ...
                    with patch.object(manager, '_downloader') as mock_downloader:
                        ...
                        mock_downloader_instance.download_segments.side_effect = quality_download
                        mock_downloader.__aenter__.return_value = mock_downloader_instance
    ...
    async def test_pattern_consistency_across_operations(self, detector):
        ...
        for original_url in test_urls:
            ...
            if match:
                ...
                regenerated_url = detector._generate_segment_url(base_url, pattern, original_number)
                
                # Should match original (or be very close)
                assert regenerated_url.split('/')[-1].split('.')[0].endswith(str(original_number))
