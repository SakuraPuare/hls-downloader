...
class TestHLSDetector:
    ...
    def test_extract_url_pattern_simple(self, detector):
        ...
        assert (
            base_url
            == "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/"
        )
        assert pattern == "{}.ts"
        assert extension == "ts"
    ...
    def test_generate_segment_url_simple(self, detector):
        ...
        assert (
            url
            == "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/5.ts"
        )
    ...
    async def test_check_segment_exists_success(self, detector):
        ...
        with patch.object(detector, "_session") as mock_session:
            ...
            exists = await detector._check_segment_exists(
        ...
        with patch.object(detector, "_session") as mock_session:
            mock_session.head = AsyncMock(
                side_effect=httpx.HTTPStatusError(
                    "Not found", request=MagicMock(), response=mock_head_response
                )
            )
            ...
            exists = await detector._check_segment_exists(
                "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/999.ts"
            )
    ...
    async def test_check_segment_exists_head_fails_get_succeeds(self, detector):
        ...
        with patch.object(detector, "_session") as mock_session:
            mock_session.head = AsyncMock(
                side_effect=httpx.RequestError("HEAD not supported")
            )
    ...
    async def test_binary_search_max_segment(self, detector):
        ...
        with patch.object(detector, "_find_upper_bound") as mock_upper:
            with patch.object(detector, "_batch_check_segments") as mock_batch:
                ...
                max_segment = await detector._binary_search_max_segment(
                    base_url, pattern
                )
        ...
        with patch.object(detector, "_binary_search_max_segment") as mock_search:
            ...
            async with detector:
                ...
                for i, segment in enumerate(segments, 1):
                    ...
                    assert (
                        segment.url
                        == f"https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/{i}.ts"
                    )
    ...
    async def test_detect_segments_without_context_manager(self, detector):
        ...
        with pytest.raises(
            RuntimeError, match="Detector must be used as async context manager"
        ):
            await detector.detect_segments(
                "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/1.ts"
            )
    ...
    def test_url_pattern_variations(self, detector):
        ...
        test_cases = [
            ...
            (
                "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/001.ts",
                "https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/",
                "{:03d}.ts",
                "ts",
            ),
            ...
            (
                "https://example.com/seg_001.ts",
                "https://example.com/",
                "seg_{:03d}.ts",
                "ts",
            ),
            (
                "https://example.com/video/chunk_001_hd.ts",
                "https://example.com/video/",
                "chunk_{:03d}_hd.ts",
                "ts",
            ),
            (
                "https://example.com/stream/1.m4s",
...
class TestBinarySearchAlgorithm:
    ...
    async def test_detect_missing_ranges_small(self, detector):
        ...
        with patch.object(detector, "_batch_check_segments") as mock_batch:
            ...
            missing_ranges = await detector._detect_missing_ranges(
                base_url, pattern, 1, 10
            )
    ...
    async def test_binary_search_with_gaps(self, detector):
        ...
        def mock_check_exists(url):
            ...
            return (
                (1 <= segment_num <= 10)
                or (15 <= segment_num <= 20)
                or (25 <= segment_num <= 30)
            )
    ...
    async def test_large_range_missing_detection(self, detector):
        ...
        with patch.object(detector, "_batch_check_segments") as mock_batch:
            with patch.object(detector, "_check_segment_exists") as mock_check:
                ...
                missing_ranges = await detector._detect_missing_ranges(
                    base_url, pattern, 50, 250, max_gap=20
                )
