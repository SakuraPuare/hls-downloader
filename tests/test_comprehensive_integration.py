...
class TestEndToEndDownload:
    ...
    async def test_complete_download_workflow(self, real_test_urls, e2e_config):
        """Test complete download workflow with minimal segments."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = DownloadManager(e2e_config)

            # Create a single test segment to minimize download
            test_segments = [
                SegmentInfo(
    ...
    async def test_resume_workflow_end_to_end(self, real_test_urls, e2e_config):
        """Test resume functionality with minimal segments."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = DownloadManager(e2e_config)

            # Use only 2 segments to minimize download
            segments = [
                SegmentInfo(
                    url=real_test_urls["cntv_sample"],
                    index=1,
                    filename="segment_000001.ts",
                    downloaded=True,  # First segment already downloaded
                ),
                SegmentInfo(
                    url=real_test_urls["cntv_sample"],
                    index=2,
                    filename="segment_000002.ts",
                    downloaded=False,  # Second segment needs download
                ),
            ]
...
class TestNetworkExceptionSimulation:
    ...
    def network_config(self):
        ...
        return DownloadConfig(
            max_concurrent=2, max_retries=2, timeout=5, auto_merge=False
        )
    ...
    async def test_connection_timeout_simulation(self, network_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_segments = [
                SegmentInfo(
                    url="http://timeout.example.com/segment1.ts",
                    index=1,
                    filename="segment1.ts",
                ),
                SegmentInfo(
                    url="http://timeout.example.com/segment2.ts",
                    index=2,
    ...
    async def test_http_error_codes_simulation(self, network_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for status_code, _reason in error_scenarios:
                test_segments = [
                    SegmentInfo(
    ...
    async def test_network_interruption_recovery(self, network_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_segments = [
                ...
                SegmentInfo(
                    url="http://unstable.example.com/segment2.ts",
                    index=2,
                    filename="segment2.ts",
                ),
                SegmentInfo(
                    url="http://unstable.example.com/segment3.ts",
                    index=3,
    ...
    async def test_dns_resolution_failure(self, network_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_segments = [
                SegmentInfo(
                    url="http://nonexistent.invalid/segment1.ts",
                    index=1,
                    filename="segment1.ts",
                )
            ]
...
class TestPerformanceBenchmarks:
    ...
    def benchmark_config(self):
        ...
        return DownloadConfig(
            max_concurrent=5, max_retries=1, timeout=10, auto_merge=False
        )

    @pytest.mark.asyncio
    async def test_concurrent_download_performance(self, benchmark_config):
        """Benchmark concurrent download performance with minimal segments."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Reduce segment count to minimize testing overhead
            segment_count = 3
            test_segments = [
                SegmentInfo(
            ...
            with patch.object(manager, "_detector") as mock_detector:
                ...
                with patch.object(manager, "_downloader") as mock_downloader:
                    ...
                    async def benchmark_download(segments, output_dir):
                        ...
                        download_time = (
                            0.05 * len(segments) / benchmark_config.max_concurrent
                        )
                    ...
                    with (
                        ...
                        # Performance assertions - adjusted for minimal segments
                        total_time = end_time - start_time
                        assert total_time < 2.0, (
                            f"Download took {total_time:.2f}s, expected < 2.0s"
                        )
    ...
    async def test_memory_usage_efficiency(self, benchmark_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Reduce segments for memory efficiency testing
            segment_count = 5
            test_segments = [
                SegmentInfo(
            ...
            with patch.object(manager, "_detector") as mock_detector:
                ...
                with patch.object(manager, "_downloader") as mock_downloader:
                    ...
                    with (
                        ...
                        # Memory increase should be reasonable (< 50MB for minimal test)
    ...
    async def test_scalability_with_large_segment_count(self, benchmark_config):
        """Test scalability with moderate number of segments."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Reduce segment count for faster testing
            segment_count = 10
            test_segments = [
                SegmentInfo(
            ...
            with patch.object(manager, "_detector") as mock_detector:
                ...
                with patch.object(manager, "_downloader") as mock_downloader:
                    ...
                    with (
                        ...
                        # Should handle moderate segment count efficiently
                        total_time = end_time - start_time
                        assert total_time < 3.0, (
                            f"Moderate scale download took {total_time:.2f}s"
                        )
...
class TestURLPatternCompatibility:
    ...
    def pattern_config(self):
        ...
        return DownloadConfig(
            max_concurrent=2, max_retries=1, timeout=5, auto_merge=False
        )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                test_segments = [
                    SegmentInfo(
                        ...
                        if "{}" in pattern
                        else pattern.replace("{:0", "{:0").format(i),
                        index=i,
                        filename=f"segment{i:03d}.ts",
                    )
                    for i in range(1, 4)
                    ...
                    with patch.object(manager, "_downloader") as mock_downloader:
                        ...
                        with (
                            ...
                            result = await manager.download_hls(
                                ...
                                output_dir=temp_dir,
                                output_filename="pattern_test.mp4",
                            )
    ...
    async def test_edge_case_patterns(self, pattern_config):
        ...
        edge_cases = [
            ...
            (
                "Large gaps",
                ["http://example.com/segment1.ts", "http://example.com/segment100.ts"],
            ),
            (
                "Non-sequential",
                [
                    "http://example.com/segment5.ts",
                    "http://example.com/segment3.ts",
                    "http://example.com/segment7.ts",
                ],
            ),
        ]
        ...
        for case_name, urls in edge_cases:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                test_segments = [
                    SegmentInfo(url=url, index=i + 1, filename=f"segment{i + 1:03d}.ts")
...
class TestUserScenarioRegression:
    ...
    async def test_typical_user_workflow(self, user_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Reduce typical segment count for testing
            test_segments = [
                SegmentInfo(
                ...
                with patch.object(manager, "_downloader") as mock_downloader:
                    ...
                    async def typical_download(segments, output_dir):
                        ...
                        for i, segment in enumerate(segments):
                            if i == 1:  # One segment fails initially
                                segment.downloaded = False
                            else:
                                segment.downloaded = True
                        return segments
    ...
    async def test_power_user_scenario(self, user_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Moderate segments for power user testing
            test_segments = [
                SegmentInfo(
    ...
    async def test_mobile_user_scenario(self, user_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Minimal segments for mobile testing
            test_segments = [
                SegmentInfo(
    ...
    async def test_batch_download_scenario(self, user_config):
        ...
        for i, url in enumerate(video_urls):
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                test_segments = [
                    SegmentInfo(
                        url=url.format(j), index=j, filename=f"segment{j:03d}.ts"
                    )
                    for j in range(1, 3)  # 2 segments per video
                ]
