...
class TestAsyncDownloader:
    ...
    @pytest.mark.asyncio
    async def test_download_segments_http_error(
        self, config, sample_segments, temp_dir
    ):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            failed_segment = next((s for s in sample_segments if s.index == 2), None)
    ...
    async def test_download_resume_server_no_support(self, config, temp_dir):
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            with patch.object(
                downloader._client, "stream", return_value=MockResponse()
            ):
                result = await downloader.download_single_segment_with_retry(
                    segment, temp_dir
                )
        ...
        async with AsyncDownloader(config) as downloader:
            ...
            class MockResponse:
                def __init__(self, content, reported_size=None):
                    ...
                    self.headers = {
                        "content-length": str(reported_size or len(content))
                    }
    ...
    async def test_download_segments_with_mixed_results(self, config, temp_dir):
        ...
        segments = [
            ...
            SegmentInfo(
                url="https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/segment2.ts",
                index=2,
                filename="segment2.ts",
            ),
            SegmentInfo(
                url="https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/segment3.ts",
                index=3,
                filename="segment3.ts",
            ),
        ]
