...
class TestErrorScenarios:
    ...
    async def test_cascading_failures(self, error_handler, sample_segment):
        ...
        mock_operation.side_effect = [
            ...
            httpx.HTTPStatusError("500 Server Error", request=Mock(), response=Mock(status_code=500, reason_phrase="Server Error")),
            "success"
        ]
    ...
    @pytest.mark.asyncio
    async def test_max_retries_with_different_errors(self, error_handler, sample_segment):
    ...
    @pytest.mark.asyncio
    async def test_non_retryable_error_stops_immediately(self, error_handler, sample_segment):
    ...
    def test_retry_delay_calculation_edge_cases(self, error_handler):
        ...
        delay_high = error_handler.get_retry_delay(10, RetryStrategy.EXPONENTIAL_BACKOFF)
        assert delay_high > 0
        ...
        delay_negative = error_handler.get_retry_delay(-1, RetryStrategy.EXPONENTIAL_BACKOFF)
    ...
    async def test_concurrent_error_handling(self, error_handler):
        ...
        segments = [
            SegmentInfo(url=f"https://example.com/segment{i}.ts", index=i, filename=f"segment{i}.ts")
            for i in range(5)
        ]
        ...
        summary = error_handler.get_error_summary()
        assert summary["total_errors"] >= len(segments)  # At least one error per segment
...
class TestEdgeCases:
    ...
    def error_handler(self):
        """Create a test error handler."""
        return ErrorHandler(max_retries=0, base_delay=0.01)  # No retries for edge case testing
    ...
    @pytest.mark.asyncio
    async def test_operation_with_complex_return_value(self, error_handler, sample_segment):
