...
class TestMainFunction:
    ...
    async def test_main_save_config(self, capsys):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "sys.argv", ["cli.py", "--save-config", "--config", str(config_path)]
            ):
                await main()
    ...
    async def test_main_verbose_output(self, mock_manager_class, capsys):
        ...
        assert (
            "Downloading from: https://dh5wswx02.v.cntv.cn/asp/h5e/hls/1200/0303000a/3/default/6f3ab539680c4b359d857b4c73a824eb/{}.ts"
            in captured.out
        )
...
class TestIntegration:
    ...
    async def test_config_file_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "sys.argv", ["cli.py", "--config", str(config_path), "--show-config"]
            ):
                await main()
