...
class PerformanceMetrics:
    ...
    def _take_sample(self):
        ...
        if not PSUTIL_AVAILABLE or not self.process:
            ...
            self.memory_samples.append({"timestamp": time.time(), "rss": 0, "vms": 0})
            self.cpu_samples.append({"timestamp": time.time(), "cpu_percent": 0})
            return
        ...
        self.memory_samples.append(
            {"timestamp": time.time(), "rss": memory_info.rss, "vms": memory_info.vms}
        )
        self.cpu_samples.append({"timestamp": time.time(), "cpu_percent": cpu_percent})
    ...
    def get_average_cpu(self):
        ...
        if self.cpu_samples:
            return sum(sample["cpu_percent"] for sample in self.cpu_samples) / len(
                self.cpu_samples
            )
        return 0
...
class TestDetectionPerformance:
    ...
    async def test_binary_search_performance(self, performance_detector):
        ...
        assert metrics.get_duration() < 5.0, (
            f"Binary search took {metrics.get_duration():.2f}s, expected < 5.0s"
        )
    ...
    async def test_batch_check_performance(self, performance_detector):
        ...
        # Performance assertions
        assert metrics.get_duration() < 2.0, (
            f"Batch check took {metrics.get_duration():.2f}s"
        )

        # Should be faster than sequential (100 * 0.001 = 0.1s minimum sequential)
        # With concurrency, should be much faster
        assert metrics.get_duration() < 0.5, (
            "Batch check should benefit from concurrency"
        )
    ...
    async def test_detection_scalability(self, performance_detector):
        ...
        for count in segment_counts:
            ...
            segments = await performance_detector.detect_segments(
                "http://example.com/segment{}.ts"
            )
        ...
        for count in segment_counts:
            ...
            if count > 10:
                ...
                assert ratio < count / 10, (
                    f"Performance doesn't scale well for {count} segments"
                )
...
class TestDownloadPerformance:
    ...
    async def test_concurrent_download_throughput(self, performance_config):
        ...
        for concurrency in concurrency_levels:
            config = DownloadConfig(
                max_concurrent=concurrency, max_retries=1, timeout=10, auto_merge=False
            )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                async with AsyncDownloader(config) as downloader:
                    ...
                    downloaded_segments = await downloader.download_segments(
                        test_segments, temp_dir
                    )
                ...
                results[concurrency] = {
                    ...
                    "successful_segments": len(
                        [s for s in downloaded_segments if s.downloaded]
                    ),
                    "memory_mb": metrics.get_peak_memory() / 1024 / 1024,
                }

        # Verify concurrency benefits
        assert results[1]["duration"] > results[8]["duration"], (
            "Higher concurrency should be faster"
        )
        assert results[8]["throughput_mbps"] > results[1]["throughput_mbps"], (
    ...
    async def test_memory_efficiency_during_download(self, performance_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert memory_growth_mb < 100, (
                f"Memory growth {memory_growth_mb:.1f}MB too high for 100 segments"
            )
    ...
    async def test_download_speed_consistency(self, performance_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            speed_variance = sum(
                (s - avg_speed) ** 2 for s in speed_measurements
            ...
            assert coefficient_of_variation < 0.2, (
                f"Speed too inconsistent: CV={coefficient_of_variation:.3f}"
            )
...
class TestEndToEndPerformance:
    ...
    async def test_complete_workflow_performance(self, e2e_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(manager, "_detector") as mock_detector:
                ...
                mock_detector_instance.detect_segments.side_effect = (
                    mock_detect_segments
                )
            ...
            assert total_duration < 10.0, (
                f"Complete workflow took {total_duration:.2f}s, expected < 10.0s"
            )
            ...
            assert throughput_mbps > 1.0, (
                f"Throughput {throughput_mbps:.2f} MB/s too low"
            )
    ...
    async def test_resource_cleanup_performance(self, e2e_config):
        ...
        assert avg_cleanup_time < 1.0, (
            f"Cleanup took {avg_cleanup_time:.2f}s on average"
        )

        # Times should be consistent
        max_time = max(cleanup_times)
        min_time = min(cleanup_times)
        assert (max_time - min_time) / avg_cleanup_time < 0.5, (
            "Cleanup times too inconsistent"
        )
    ...
    async def test_large_scale_performance(self, e2e_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if total_duration > 0:  # Avoid division by zero
                ...
                assert segments_per_second > 1, (
                    f"Processing rate {segments_per_second:.1f} segments/s too low"
                )
...
def save_benchmark_results(results_file, test_name, metrics):
    ...
    results[test_name] = {"timestamp": time.time(), "metrics": metrics.to_dict()}
...
class TestBenchmarkReporting:
    ...
    async def _benchmark_detection(self):
        ...
        async with detector:
            await detector._binary_search_max_segment(
                "http://example.com/", "segment{}.ts"
            )
