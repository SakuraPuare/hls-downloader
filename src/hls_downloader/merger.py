...
class VideoMerger:
    ...
    def _generate_concat_file(self, segment_files: List[str]) -> str:
        ...
        try:
            ...
            logger.info(f"Generated concat file: {concat_path} with {len(segment_files)} segments")
            return concat_path
    ...
    async def merge_segments(
        ...
        try:
            ...
            logger.info(f"Successfully merged {len(segment_files)} segments to {output_file}")
    ...
    async def _run_ffmpeg_merge(
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                raise MergeError(f"ffmpeg failed with return code {process.returncode}: {error_msg}")
