...
class HLSDetector(DetectorInterface):
    ...
    def __init__(
        self, timeout: int = 30, max_concurrent_checks: int = 20, cache_ttl: int = 300
    ):
        ...
        self._missing_ranges: dict[
            str, set[tuple[int, int]]
        ] = {}  # Track known missing ranges
    ...
    def _extract_url_pattern(self, url: str) -> tuple[str, str, str]:
        ...
        for pattern in brace_patterns:
            ...
            if match:
                ...
                if len(groups) == 2:  # Simple pattern: {}.ext
                    ...
                    len(groups) == 3
                ):  # With prefix: prefix{}.ext or prefix{}suffix.ext
        ...
        for pattern in number_patterns:
            ...
            if match:
                ...
                pattern_template = (
                    f"{prefix}{number_template}{suffix}.{extension}{query_fragment}"
                )
    ...
    async def _detect_missing_ranges(
        self, base_url: str, pattern: str, start: int, end: int, max_gap: int = 10
    ) -> list[tuple[int, int]]:
        ...
        for i in range(sample_size):
            index = (
                start + (i * (end - start)) // (sample_size - 1)
                if sample_size > 1
                else start
            )
        ...
        sample_urls = [
            self._generate_segment_url(base_url, pattern, i) for i in sample_indices
        ]
    ...
    async def _binary_search_max_segment(self, base_url: str, pattern: str) -> int:
        ...
        max_consecutive_missing = (
            5  # Stop if we find this many consecutive missing segments
        )
        ...
        while left <= right:
            ...
            batch_urls = [
                self._generate_segment_url(base_url, pattern, i) for i in batch_indices
            ]
            ...
            if mid_index < len(exists_list) and exists_list[mid_index]:
                ...
                for i, exists in enumerate(exists_list):
                    ...
                    if not exists:
                        ...
                        if missing_start is not None:
                            self._add_missing_range(
                                base_url, pattern, missing_start, index - 1
                            )
                            missing_start = None
                ...
                if missing_start is not None:
                    self._add_missing_range(
                        base_url, pattern, missing_start, batch_indices[-1]
                    )
                ...
                if consecutive_missing >= max_consecutive_missing:
                    ...
                    missing_ranges = await self._detect_missing_ranges(
                        base_url, pattern, max(1, mid - 10), mid + 10
                    )
    ...
    def get_cache_stats(self) -> dict[str, int]:
        ...
        expired_entries = sum(
            1 for entry in self._cache.values() if entry.is_expired(self.cache_ttl)
        )
        missing_ranges_count = sum(
            len(ranges) for ranges in self._missing_ranges.values()
        )
