...
class ResumeValidator:
    ...
    def scan_existing_files(self, segments_dir: Path) -> Dict[str, Dict[str, any]]:
        ...
        try:
            for file_path in segments_dir.iterdir():
                if file_path.is_file() and file_path.suffix.lower() in ['.ts', '.m4s', '.mp4']:
                    try:
                        stat = file_path.stat()
    ...
    async def validate_segments(
        ...
        for segment in segments:
            ...
            if file_info['is_empty']:
                logger.debug(f"Segment {segment.index} file is empty, marking as invalid")
    ...
    async def deep_validate_segments(
        ...
        for i, result in enumerate(validation_results):
            ...
            if isinstance(result, Exception):
                logger.warning(f"Validation error for segment {segment.index}: {result}")
            ...
            try:
                ...
                try:
                    ...
                    if e.response.status_code == 404:
                        logger.warning(f"Segment {segment.index} not found on server (404)")
                        return False
                    else:
                        logger.warning(f"HTTP error validating segment {segment.index}: {e}")
                        # For other HTTP errors, assume file is valid to avoid re-downloading
                        return True
                
                except httpx.RequestError as e:
                    logger.warning(f"Network error validating segment {segment.index}: {e}")
    ...
    def get_resume_summary(
        ...
        """
        total_segments = len(valid_segments) + len(invalid_segments) + len(missing_segments)
        ...
        return {
            ...
            "completion_percentage": (len(valid_segments) / total_segments * 100) if total_segments > 0 else 0,
            "valid_bytes": valid_bytes,
            "can_resume": segments_to_download > 0 and len(valid_segments) > 0,
            "resume_benefit": (len(valid_segments) / total_segments * 100) if total_segments > 0 else 0
        }