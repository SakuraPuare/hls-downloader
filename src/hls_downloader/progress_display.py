...
class ProgressDisplay:
    ...
    def update_worker_progress(
        self, worker_id: int, increment: int, total: Optional[int] = None
    ) -> None:
        """
    ...
    def _update_main_progress_description(self) -> None:
        ...
        if self._stats.downloaded_segments > 0 and self._stats.total_segments > 0:
            progress_ratio = (
                self._stats.downloaded_segments / self._stats.total_segments
            )
            if progress_ratio > 0:
    ...
    def display_final_stats(self) -> None:
        ...
        average_speed = (
            self._stats.downloaded_bytes / total_time if total_time > 0 else 0
        )
    ...
    def is_active(self) -> bool:
        ...
        with self._lock:
            return self._main_progress is not None or len(self._worker_progresses) > 0
