...
class DownloadManager:
        ...
        try:
            ...
            if not self._current_state or not self._current_state.segments:
                ...
                if self._current_state:
                    self._state_manager.update_state_segments(self._current_state, segments)
                    self._state_manager.update_state_status(self._current_state, "downloading")
            else:
                logger.info(f"Using {len(self._current_state.segments)} segments from saved state")
            ...
            if self._current_state:
                self._state_manager.update_state_status(self._current_state, "completed")
            ...
            if self._current_state and self._state_manager:
                try:
                    self._state_manager.update_state_status(self._current_state, "failed")
    ...
    async def _validate_resume_state(self) -> Dict[str, Any]:
        ...
        async with self._resume_validator:
            # Validate segments
            valid_segments, invalid_segments, missing_segments = await self._resume_validator.validate_segments(
            ...
            for segment in self._current_state.segments:
                segment.downloaded = any(v.index == segment.index for v in valid_segments)
    ...
    async def _download_segments_with_resume(self, segments: List[SegmentInfo]) -> List[SegmentInfo]:
    ...
    async def resume_download(
        ...
        return await self.download_hls(url, output_dir, output_filename, force_restart=False)
